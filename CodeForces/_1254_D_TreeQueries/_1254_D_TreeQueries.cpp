...
int main(){
	...
	while(Q--){
		...
		if(c == 1){
			...
			val[n] = add(val[n], d);
			if(!ishvy[n]){
				upd(1, d-mult(d, mult(invN, N-sz[n])));
				upd(lft[n], mult(d, mult(invN, N-sz[n])));
				upd(rht[n]+1, -mult(d, mult(invN, N-sz[n])));
				for(int e : graph[n]){
					if(e != par[n]){
						upd(lft[e], -mult(d, mult(invN, sz[e])));
						upd(rht[e]+1, mult(d, mult(invN, sz[e])));
					}
				}
			}
		}
